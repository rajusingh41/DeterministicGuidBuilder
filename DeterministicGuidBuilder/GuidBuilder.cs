...
namespace DeterministicGuidBuilder
{
    public static class GuidBuilder
    {
        ...
        /// <summary>
        /// The namespce for fully-qualified domain names (from FRC 4122, Appendix C).
        /// </summary>
        private static readonly Guid DefaultNameSpace = new("6ba7b811-9dad-11d1-80b4-00c04fd430c8");
        ...
        public static Guid Create(string name) => Create(DefaultNameSpace, name, UuidVersion.SHA256Hashing);
        ...
        /// <param name="name">The name (within that namespace).</param>
        /// <returns>A UUID derived from the namespace and name.</returns>
        public static Guid Create(Guid namespaceId, string name) => Create(namespaceId, name, UuidVersion.SHA256Hashing);
        ...
        /// <returns>A UUID derived from the namespace and name.</returns>
        public static Guid Create(Guid namespaceId, string name, UuidVersion version)
        {
            if (name is null)
            ...
            return Create(namespaceId, Encoding.UTF8.GetBytes(name), (int)version);
        }
        ...
        /// <returns>A UUID derived from the namespace and name.</returns>
        public static Guid Create(Guid namespaceId, byte[] nameBytes) => Create(namespaceId, nameBytes, (int)UuidVersion.SHA256Hashing);
        ...
        private static Guid Create(Guid namespaceId, byte[] nameBytes, int version)
        {
            ...
            byte[] hash;
            using (var algorithm = version == 3 ? (HashAlgorithm)MD5.Create() : SHA256.Create())
